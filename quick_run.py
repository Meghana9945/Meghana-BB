...
print(f"✅ Cleaned {len(df)} records\n")
# ================================================================
# 🧩 USER BEHAVIOR PROFILING — NEW FEATURE
# Building distinct profiles for each user to detect deviations
# ================================================================
print("👤 Step 2.5: Building User Behaviour Profiles...")
print("-"*70)

# For each user, calculate normal behaviour statistics
user_profiles = df.groupby('user_id_original').agg({
    'files_accessed': ['mean', 'std'],
    'failed_logins': ['mean', 'std'],
    'data_outbound_mb': ['mean', 'std'],
    'cpu_usage': ['mean', 'std'],
    'file_encryption_rate': ['mean', 'std'],
    'session_duration': ['mean', 'std'],
}).fillna(0)

# Flatten column names
user_profiles.columns = ['_'.join(col) for col in user_profiles.columns]
user_profiles.reset_index(inplace=True)

# Merge profiles back into main dataframe
df = df.merge(user_profiles, on='user_id_original', how='left')

# Create deviation features (how much user deviates from their baseline)
df['dev_files_accessed'] = abs(df['files_accessed'] - df['files_accessed_mean']) / (df['files_accessed_std'] + 1)
df['dev_failed_logins'] = abs(df['failed_logins'] - df['failed_logins_mean']) / (df['failed_logins_std'] + 1)
df['dev_outbound'] = abs(df['data_outbound_mb'] - df['data_outbound_mb_mean']) / (df['data_outbound_mb_std'] + 1)
df['dev_cpu'] = abs(df['cpu_usage'] - df['cpu_usage_mean']) / (df['cpu_usage_std'] + 1)
df['dev_encryption'] = abs(df['file_encryption_rate'] - df['file_encryption_rate_mean']) / (df['file_encryption_rate_std'] + 1)
df['dev_session'] = abs(df['session_duration'] - df['session_duration_mean']) / (df['session_duration_std'] + 1)

print("✅ User behaviour profiles created")
print("✅ Deviation features added for each user\n")
